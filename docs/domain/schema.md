...
| ID                 | **`INTEGER PRIMARY KEY AUTOINCREMENT`（SQLite）/ `BIGINT GENERATED BY DEFAULT AS IDENTITY`（PostgreSQL）**。Rust では `i64`。Banto の既存リソース（`items` / `users` / `attachments`）が全てこの形（`conventions.md` §11） |
...
| `id`                        | `INTEGER PK AUTOINCREMENT`                        |                                                                                                                  |
| `project_id`                | `INTEGER NOT NULL REFERENCES projects(id)`        |                                                                                                                  |
| `trip_id`                   | `INTEGER REFERENCES trips(id) ON DELETE SET NULL` | 出張から生成された場合                                                                                           |
...
| --------------------------- | ------------------------------------------- | ---------------------------------------------- |
| `id`                        | `INTEGER PK AUTOINCREMENT`                  |                                                |
| `customer_id`               | `INTEGER NOT NULL REFERENCES customers(id)` | **`invoice_id` は持たない**（`CLAUDE.md` 1.4） |
...
方式の全体は `docs/domain/sync.md`。ここは表の一覧のみ。

| 表               | 何を持つ                                                           | マイグレーション |
| ---------------- | ------------------------------------------------------------------ | ---------------- |
| `sync_outbox`    | 変わった行の目次（`seq` / テーブル / 主キー / 操作）。値は持たない | 0022             |
| `sync_state`     | 相手端末ごとの進捗と最終同期日時                                   | 0022             |
| `sync_conflicts` | **未解決の衝突**。両側の行を `SyncRow` の JSON で持つ              | 0025             |

`sync_conflicts` が要るのは、差し戻された衝突が次の同期で再び送られて
こないため（`docs/domain/sync.md` 11.7）。進捗は衝突があっても進むので、
受け取った側が保存しない限りその編集は二度と現れない。

外部キーを張らず、行の中身を JSON で持つ。片側が墓石（削除済み）でも
保持する必要があり、8テーブルぶんの異なる列構成を型付きの列で受けると
目録が増えるたびにこの表も変わるため。検索も集計もしない一時的な控え。
