...
### 4.2 `embed-ui` を入れる（2026-08-22 方針変更）

`embed-ui` は「LAN のブラウザへ実物の画面を配る」ための機能。Android 版が
自分の webview に表示する画面は `frontendDist` から入るので、この feature が
効くのは**埋め込みサーバーが LAN のブラウザへ返す中身だけ**。

当初は「スマホ側で `banto-serve` 相当を動かす必要は無い」として外していた
（話しかけるのは常にスマホ側、`docs/domain/sync.md` 11.1）。**スマホ母艦案**
（スマホの DB を正本にし、PC や別端末はブラウザでスマホへ繋ぐ。同期を
不要にする）を検討することになり、その実験にはスマホがブラウザへ実物の
画面を配れる必要があるため、APK に同梱するよう変えた。

- 無効のままだと、埋め込みサーバーはプレースホルダのページしか返さない
  （`core/src/assets.rs`）
- サイズ増は数 MB で、62MB の APK では誤差の範囲
- **母艦案を採るかどうかはまだ決めていない。** 決め手は「画面オフ・
  バックグラウンドでサーバが生き続けるか」で、これは実機でしか分からない。
  実験の結果と判断は `docs/domain/sync.md` に記録する
...
`.github/workflows/android-release.yml`。7章の `android-build.yml`
（デバッグ署名・使い捨て）とは別のワークフローファイル。

### 8.1 なぜ方針転換したか

7.2 の「署名しない」判断は、**ストアを経由せず自分の端末へ直接入れる・
使い捨てデータの実験機**という前提（8節、当時の番号）の上に立っていた。
デバッグ署名は毎回鍵が変わるので、更新のたびに一度アンインストールが要る
——実験機なら実害は無い。

スマホ母艦化（`docs/domain/sync.md` 0節：スマホの DB を正本にする案）の
検討で、この前提が崩れた。**実データを持つ端末へ継続配布する**運用になると、
更新のたびのアンインストール = 母艦DBの消去になり、7.2 の判断はそのままでは
使えない。そのため 2026-08-25（アルファ）、**固定鍵で署名したリリース
APK** を別経路として持つことにした。7章のデバッグ経路自体の方針は変えて
いない——「手元で試すだけ」の用途では依然としてデバッグ署名で足りる。

### 8.2 キーストアを作る（ユーザーが手元で一度だけ）

**リポジトリにコミットしない。値をドキュメント・Issue・コミットメッセージに
書かない**（`CLAUDE.md` 第8章：公開リポジトリ）。自分の PC で一度だけ実行し、
生成された `.jks` ファイルと入力したパスワード・エイリアスは自分だけが
知っている場所（パスワードマネージャ等）に控える。

```sh
keytool -genkeypair -v \
  -keystore banto-business-release.jks \
  -keyalg RSA -keysize 2048 -validity 10000 \
  -alias <自分で決めるエイリアス名>
```

対話式で以下を聞かれる。すべて自分で決めた値を入れる（本ドキュメントには
サンプル値も書かない——実在の値と誤認されないため）。

- キーストアのパスワード
- 氏名・組織などの識別情報（DN。適当で構わない。Android の署名検証は鍵の
  同一性のみ見る）
- 鍵（エイリアス）のパスワード（キーストアと同じでもよい）

`banto-business-release.jks` はどこにもコミットしない。**失うと最初から
作り直しになり、その場合はまた 8.4 の①②（アンインストール）が発生する**
——バックアップを取っておくこと。

### 8.3 GitHub Secrets に登録する

リポジトリの Settings → Secrets and variables → Actions で、以下の4つを
登録する（ワークフロー側はこれらを参照するだけで、登録作業自体はユーザーが
GitHub の画面から行う）。

| Secret 名                  | 値                                          |
| --------------------------- | -------------------------------------------- |
| `ANDROID_KEYSTORE_B64`      | `.jks` ファイルを base64 化した文字列（`base64 -w0 banto-business-release.jks` の出力） |
| `ANDROID_KEYSTORE_PASSWORD` | キーストアのパスワード                       |
| `ANDROID_KEY_ALIAS`         | 鍵のエイリアス名                             |
| `ANDROID_KEY_PASSWORD`      | 鍵（エイリアス）のパスワード                 |

4つ揃っていないと `android-release.yml` はタグ経路で fail する（8.5）。

### 8.4 リリース手順

1. 8.3 の Secrets を登録済みであることを確認する
2. タグを付けて push する
   ...
   タグ名は `scripts/check-versions.mjs --tag` の慣例に合わせ `v` +
   マニフェストの version（`apps/admin-template/package.json` 等。Task 1 で
   `0.1.0` に仕切り直し済み）を先頭に、アルファ番号を後ろに付ける
   （`v0.1.0-alpha.1`, `v0.1.0-alpha.2`, …）
3. Actions タブで `Android release` の run を確認する。成功すると
   **ドラフトの** GitHub Release が作られ、署名済み APK
   （`banto-business-<タグ名>-aarch64.apk`）が添付される
4. Release ページの内容を確認し、公開する（ドラフトのままでは配布されない
   ——**公開操作はユーザーが行う**、ワークフローは自動公開しない）
5. スマホのブラウザで Release ページを開き、APK をダウンロードして開く
   （「提供元不明のアプリ」の許可が必要、5節）

### 8.5 注意事項

1. **初回のみ、既存アプリのアンインストールが必要。** 7.2 のデバッグ署名
   から 8.1 の固定鍵署名へ切り替わるため、署名が変わる。Android は
   「同じ鍵で署名され続けた APK」しか上書き更新できない。**データが入って
   いる端末では、アンインストールの前に必ず M17 のバックアップを取る**
   （`/backup` 画面、`docs/plan.md` M17）
2. 2回目以降は、同じ鍵で署名され続ける限り**上書きインストールでデータが
   維持される**。鍵を使い回すこと自体が固定鍵署名へ切り替えた目的
3. **キーストアを失うと、また①のアンインストールが発生する。** 8.2 で
   作ったキーストアは唯一の原本であり、GitHub 側にも平文の控えは置かない
   （Secrets は書き込み専用で読み出せない）。手元での保管に注意する
4. `versionCode`（Tauri が `tauri.conf.json` の `version` から導出する
   Android 側のビルド番号）は、Task 1 のバージョン仕切り直しで `1.2.0` 系
   から `0.1.0` 系へ下がる。Android は通常 `versionCode` の逆行を拒むが、
   ①のとおりどのみち初回はアンインストールしてから入れ直すため、実害は無い

### 8.6 タグ / `workflow_dispatch` / PR での挙動の違い

| トリガー | secrets 有無 | 署名 | Release |
| --- | --- | --- | --- |
| タグ push (`v*`) | 揃っている | する | ドラフト作成・APK 添付 |
| タグ push (`v*`) | 欠けている | （fail） | 作らない。ビルド前に fail-fast |
| `workflow_dispatch` | 揃っている | する | 作らない。artifact アップロードのみ |
| `workflow_dispatch` | 欠けている | skip（unsigned のまま） | 作らない |
| `pull_request`（このワークフロー自身の変更） | 通常は欠けている（fork からの PR は GitHub の既定で secrets が渡らない） | skip（unsigned のまま） | 作らない |
