...
### [2026-08-21] Phase 8 — Grid に列の表示切替が無く、要らない列は「消す」しかない

| 項目 | 内容 |
|---|---|
| 事象 | `GridColumn` に表示/非表示の概念が無く、`columnsFromSchema` にも列を除く指定が無い。**「定義は持つが既定では出さない」が表現できない**ので、内部 id や自動採番コードのような「普段は読む必要が無いが、たまに要る」列を出さないためには、列そのものを組み立て時に取り除くしかない。取り除くと利用者が戻す手段も無くなる |
| 影響 | Business 固有ではない。業務アプリの一覧は「全部見せると読めない／隠すとたまに困る」の間で必ず揺れる。スマホでは列1つの幅が実際に効く（Phase 8 で実機を触って痛感した）。`ColumnMap` や `columnsFromSchema` という良い導出の仕組みがあるのに、**出す/出さないだけが呼び出し側の手作業**になっている |
| 回避策 | app 層で `.filter((column) => column.id !== 'code')` して落とし、手書きの `ID` 列は定義ごと削除した。落とした事実と「切替が入ったら既定非表示へ変えること」をコード内コメントに残してある |
| 分類 | Banto共通 |
| Banto Issue | 起票予定（`tyaro/banto`） |
| 状態 | 回避済み。あるべき形は `GridColumn.hidden?: boolean`（既定 false）＋ 利用者が切り替える UI ＋ その選択の永続化（`ui-settings` に既にテーマ等を保存する仕組みがある）。**列の順序も同じ問題を抱えている** —— `columnsFromSchema` はスキーマの定義順そのままで、順を変えるにはスキーマ側を並べ替えるしかなく、フォームの入力順と一覧の列順が連動してしまう（今回、コード欄をフォーム末尾へ動かしたら一覧でも末尾へ動いた） |

---

### [2026-08-30] Phase 8 — Grid の操作列にアイコンボタンを置けず、テキストリンクで代替

| 項目 | 内容 |
|---|---|
| 事象 | `GridColumn.cell` は `(row) => { text: string; href?: string }` しか返せず、コンポーネントや Snippet を渡す経路が無い。一覧の先頭「操作」列にアイコンボタン（lucide の外部リンクアイコン等）を置きたかったが、`cell` では文字列しか描画できない |
| 影響 | Business 固有ではない。一覧から詳細への導線をアイコンで示すのは業務アプリで一般的な UI パターンで、Industrial 側でも同様の需要が起こりうる |
| 回避策 | アルファ実機フィードバック（7件の一覧画面で「操作」列が空セルのままだった点の指摘）を受け、`cell: (row) => ({ text: m['resource.openRow'](), href: ... })` によるテキストリンク（「開く ›」）で代替した。`onRowClick`（行ダブルクリックでの遷移）はそのまま残し、リンクは追加の導線とした |
| 分類 | Banto共通 |
| Banto Issue | 未起票 |
| 状態 | 回避済み。Grid 側に `cell` の Snippet 対応（またはアイコンボタン付き操作列プリセット）が入ったら、7一覧のテキストリンクをアイコンボタンへ置き換えたい |
...
| 8 | 7 | 7 | 複数デバイスでのオフライン利用・同期の足場が無い。一覧の基底 SELECT に固定条件を足せない。モバイル標的でビルドが通らない（依存・エントリポイントの2件）。端末固有の初期化を走らせる場所が無い。シェルがセーフエリアを避けていない。Grid に列の表示切替が無い |
