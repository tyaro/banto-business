<script lang="ts">
	/**
	 * 出張一覧（docs/recipes/add-resource.md 手順8）。
	 *
	 * クライアント/サーバー両モードの切り替えは持たず、**サーバー
	 * モードのみ**にしている: 出張・案件は「1万件のデモデータを手元で
	...
	const baseColumns: GridColumn<TripRow>[] = [
		{
			id: 'open',
			header: m['resource.colActions'](),
			accessor: () => '',
			cell: (row) => ({ text: m['resource.openRow'](), href: `${base}/trips/${row.id}` }),
			width: 80,
			editable: false
		},
	...
	const gridState = new GridState<TripRow>(columns);
	const windowed = createWindowedListResource<TripRow>('trips');
	...
	function openRow(row: TripRow) {
		goto(`${base}/trips/${row.id}`);
	}
</script>

<div class="page">
	<PageHeader title={m['trips.resourceLabel']()} description={m['trips.description']()}>
		{#snippet actions()}
			{#if canWrite}
				<a class="banto-btn banto-btn--primary" href={`${base}/trips/new`}>
					<Plus size={16} />
					{m['resource.create']()}
				</a>
			{/if}
