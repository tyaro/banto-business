<script lang="ts">
	/**
	 * trips の編集・削除（docs/recipes/add-resource.md 手順8）。
...
<div class="page">
	...
	<div class="form-panel">
		{#if isNotFoundError}
			...
			>
				{#snippet action()}
					<a class="banto-btn banto-btn--secondary" href={`${base}/trips`}
			...
			<ErrorState title={m['resource.loadError']()} description={m['resource.loadErrorDesc']()}>
				{#snippet action()}
					<div class="error-actions">
						...
						<a class="banto-btn banto-btn--ghost" href={`${base}/trips`}
							>{m['common.backToList']()}</a
						>
					</div>
				{/snippet}
