<script lang="ts">
	/**
	 * customers の編集・削除（docs/recipes/add-resource.md 手順8）。
	...
	const resource = getResource('customers');
	...
	async function handleDelete() {
		...
		if (removed) goto(`${base}/customers`);
	}
</script>
...
<div class="page">
	...
	<div class="form-panel">
		{#if isNotFoundError}
			...
			>
				{#snippet action()}
					<a class="banto-btn banto-btn--secondary" href={`${base}/customers`}
			...
			<ErrorState title={m['resource.loadError']()} description={m['resource.loadErrorDesc']()}>
				{#snippet action()}
					<div class="error-actions">
						...
						<a class="banto-btn banto-btn--ghost" href={`${base}/customers`}
							>{m['common.backToList']()}</a
						>
					</div>
				{/snippet}
