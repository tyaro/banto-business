<script lang="ts">
	/**
	 * 工数一覧（docs/recipes/add-resource.md 手順8）。
	 *
	 * クライアント/サーバー両モードの切り替えは持たず、**サーバー
	 * モードのみ**にしている: 工数・案件は「1万件のデモデータを手元で
	...
	/** 分を「1h30m」の形にする。表示整形だけで、計算はしない。 */
	const minutesFormat = (value: unknown): string => {
	...
	const baseColumns: GridColumn<WorkLogRow>[] = [
		{
			id: 'open',
			header: m['resource.colActions'](),
			accessor: () => '',
			cell: (row) => ({ text: m['resource.openRow'](), href: `${base}/work-logs/${row.id}` }),
			width: 80,
			editable: false
		},
		...columnsFromSchema<WorkLogRow>(workLogsSchema, {
			overrides: {
	...
	const gridState = new GridState<WorkLogRow>(columns);
	const windowed = createWindowedListResource<WorkLogRow>('work_logs');
	...
	function openRow(row: WorkLogRow) {
		goto(`${base}/work-logs/${row.id}`);
	}
</script>

<div class="page">
	<PageHeader title={m['workLogs.resourceLabel']()} description={m['workLogs.description']()}>
		{#snippet actions()}
			{#if canWrite}
				<a class="banto-btn banto-btn--primary" href={`${base}/work-logs/new`}>
					<Plus size={16} />
					{m['resource.create']()}
				</a>
			{/if}
