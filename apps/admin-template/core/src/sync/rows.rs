...
/// 主キー1件を読む（無ければ `None`）。
pub async fn read_row(db: &Db, spec: &TableSpec, key: &str) -> Result<Option<SyncRow>, BantoError> {
    let keys = [key.to_string()];
    Ok(read_rows(db, spec, &keys).await?.into_iter().next())
}

/// 受け取った行が目録どおりの形をしているか。
///
/// **足りない列を既定値で埋めない。** 埋めると、送り手の不具合で列が落ちた
/// ときに「0 円」「NULL」が正しい値として書き込まれ、しかも同期は成功したよう
/// に見える。金額列でこれは踏みたくない。
pub fn validate_row(spec: &TableSpec, row: &SyncRow) -> Result<(), BantoError> {
    for (name, kind) in spec.columns {
        match (kind, row.values.get(*name)) {
            (_, None) => {
                return Err(malformed(spec, format!("列 {name} が無い")));
            }
            (ColumnKind::Int, Some(SyncValue::Text(_))) => {
                return Err(malformed(spec, format!("列 {name} は整数のはず")));
            }
            (ColumnKind::Text, Some(SyncValue::Int(_))) => {
                return Err(malformed(spec, format!("列 {name} は文字列のはず")));
            }
            _ => {}
        }
    }
    if !row.values.contains_key(spec.key) {
        return Err(malformed(spec, format!("主キー {} が無い", spec.key)));
    }
    Ok(())
}
...
/// 目録の全列が同値か。
///
/// **同期が収束する条件そのもの。** 取り込んだ行はトリガで outbox に載り、
/// 次の同期で送り主へ返る。そこで「同値なら書かない」が効かないと、
/// 両端末が永久に同じ行を送り合う。`created_at` / `updated_at` を送り手の値の
/// まま書き込む（[`apply_row`]）のも同じ理由。
pub fn values_equal(spec: &TableSpec, a: &SyncRow, b: &SyncRow) -> bool {
    spec.columns
        .iter()
        .all(|(name, _)| a.values.get(*name) == b.values.get(*name))
}

/// 受け取った行を書き込む。`exists` が真なら UPDATE、偽なら INSERT。
///
/// **値は送り手のものをそのまま書く。** `updated_at` をこちらの時計で
/// 上書きしない —— 上書きすると行が永久に一致せず、[`values_equal`] の
/// 収束条件が壊れる。日時は行そのものの属性で、どの端末で見ても同じであるべき。
///
/// id も送り手の値のまま入れる。相手のレンジ（`docs/domain/sync.md` 3.1）から
/// 出た値なので、こちらの採番と衝突しない。PostgreSQL の
/// `GENERATED BY DEFAULT AS IDENTITY` は明示 id を受け付ける（`ALWAYS` では
/// ないのはこのため）。
pub async fn apply_row(
    ...
    validate_row(spec, row)?;
    let dialect = db.dialect();

    // 束縛する値を先に組み立てる。方言ごとの分岐で同じ判断を2回書かないため。
    let mut binds: Vec<Bind> = Vec::new();
    let mut push = |name: &str| -> Result<(), BantoError> {
        let kind = spec
            .kind_of(name)
            .ok_or_else(|| malformed(spec, format!("列 {name} は目録に無い")))?;
        let value = row
            .values
            .get(name)
            .ok_or_else(|| malformed(spec, format!("列 {name} が無い")))?;
        binds.push(match (kind, value) {
            (ColumnKind::Int, SyncValue::Null) => Bind::Int(None),
            (ColumnKind::Int, SyncValue::Int(v)) => Bind::Int(Some(*v)),
            (ColumnKind::Text, SyncValue::Null) => Bind::Text(None),
            (ColumnKind::Text, SyncValue::Text(v)) => Bind::Text(Some(v.clone())),
            _ => return Err(malformed(spec, format!("列 {name} の型が目録と違う"))),
        });
        Ok(())
    };

    let sql = if exists {
        // 主キーは WHERE で使うので SET には並べない。
        ...
        for (name, _) in spec.columns {
            ...
            push(name)?;
            assignments.push(format!("{name} = {}", dialect.placeholder(placeholder)));
            placeholder += 1;
        }
        push(spec.key)?;
        format!(
            ...
            assignments.join(", "),
            spec.key,
            dialect.placeholder(placeholder)
        )
    } else {
        for (name, _) in spec.columns {
            push(name)?;
        ...
        format!(
            ...
            spec.name,
            spec.column_list(),
            dialect.placeholders(spec.columns.len())
        )
    };
...
/// 方言分岐の前に決めておく束縛値。同じ判断を SQLite / PostgreSQL の
/// 両方で書かないための入れ物。
enum Bind {
    Int(Option<i64>),
    Text(Option<String>),
...
fn malformed(spec: &TableSpec, message: String) -> BantoError {
    BantoError::Validation {
        field_errors: vec![FieldError {
            field: spec.name.to_string(),
            message: format!("受け取った行が壊れている: {message}"),
        }],
    }
}
