...
use admin_template_core::sync::rows::{
    read_row, table_spec, ColumnKind, SyncRow, SyncValue, SYNCED_TABLES,
};
...
/// 取り込み（`push`）が PostgreSQL でも同じように動くこと。
///
/// SQLite 側と役割が違うのは3点。
/// - 明示 id の INSERT（`GENERATED BY DEFAULT AS IDENTITY` が受け付けること）
/// - `sync_state` の UPSERT（`ON CONFLICT ... GREATEST`。SQLite は `max`）
/// - `NULL` を含む列の束縛（PostgreSQL は型に厳格で、`None::<i64>` と
///   `None::<String>` を取り違えると落ちる）
#[tokio::test]
async fn sync_push_applies_and_detects_conflicts_on_postgres() {
    let Ok(url) = std::env::var("BANTO_TEST_PG_URL") else {
        eprintln!("pg_smoke: BANTO_TEST_PG_URL unset - skipping (no PostgreSQL server)");
        return;
    };
    ...
    let incoming = SyncRow {
        ...
        key: "1000000001".to_string(),
        values: [
            ("id", SyncValue::Int(1_000_000_001)),
            ("code", SyncValue::Text("PG-P001".to_string())),
            ("name", SyncValue::Text("架空商事".to_string())),
            ("contact_person", SyncValue::Null),
            ("address", SyncValue::Null),
            ("phone", SyncValue::Null),
            ("email", SyncValue::Null),
            ("billing_name", SyncValue::Null),
            ("closing_day", SyncValue::Int(99)),
            ("payment_month_offset", SyncValue::Int(1)),
            ("payment_day", SyncValue::Int(99)),
            ("note", SyncValue::Null),
            ("created_at", SyncValue::Text("2026-08-21".to_string())),
            ("updated_at", SyncValue::Text("2026-08-21".to_string())),
            ("deleted_at", SyncValue::Null),
        ]
        .into_iter()
    ...
    let request = |rows: Vec<PushRow>, through_seq: i64, pulled_through_seq: i64| PushRequest {
        peer_device_id: 1,
        pulled_through_seq,
        through_seq,
        rows,
    ...
    let applied = sync
        ...
        .await
        .expect("push on postgres");
    assert_eq!(applied.applied, 1);
    assert_eq!(applied.received_through_seq, 5);

    // 値が送り主のまま入っていること（日時をこちらの時計で上書きしない）。
    let landed = read_row(&db, spec, "1000000001")
        .await
        .expect("read")
        .expect("row");
    assert_eq!(landed, incoming);
    ...
    // --- sync_state の UPSERT が引き上げだけを行うこと ---
    assert_eq!(again.received_through_seq, 6);
    let rewound = sync
        .push(request(vec![], 2, 0))
        .await
        .expect("push with an older watermark");
    assert_eq!(
        rewound.received_through_seq, 6,
        "進捗を巻き戻さないこと（GREATEST）"
    );
    ...
    edited.values.insert(
        "name".to_string(),
        SyncValue::Text("スマホ側で改称".to_string()),
    );
    let conflicted = sync
        .push(request(
            vec![PushRow {
        ...
        .await
        .expect("push conflicting");
    assert_eq!(conflicted.applied, 0);
    assert_eq!(conflicted.conflicts.len(), 1);
    assert_eq!(conflicted.conflicts[0].reason, ConflictReason::BothChanged);
